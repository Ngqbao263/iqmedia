            ...
            <div class="mb-3">
                ...
                <select class="form-select" id="main_category" name="main_category" required>
                    ...
                    <option value="Thương mại"
                        {{ strpos($post->category, 'Loa BF Audio') !== false || strpos($post->category, 'Karaoke gia đình') !== false || strpos($post->category, 'Bộ quản lý nguồn') !== false || strpos($post->category, 'Amply liền Mixer') !== false || strpos($post->category, 'Loa Boutum') !== false || strpos($post->category, 'Loa Latop (USA)') !== false || strpos($post->category, 'Micro BF Audio') !== false || strpos($post->category, 'Mixer BF Digital Karaoke') !== false || strpos($post->category, 'Power Amplir') !== false || strpos($post->category, 'Trọn bộ karaoke') !== false ? 'selected' : '' }}>
                        Thương mại</option>
                    <option value="Quảng Cáo"
                        {{ strpos($post->category, 'Gia Công CNC - LASER') !== false || strpos($post->category, 'Thi Công Quảng Cáo') !== false || strpos($post->category, 'In Ấn Kỹ Thuật Số') !== false || strpos($post->category, 'Thiết Kế Quảng Cáo') !== false ? 'selected' : '' }}>
                        Quảng Cáo</option>
                    <option value="Dịch vụ"
                        {{ strpos($post->category, 'Cho thuê màn hinh ánh sáng') !== false || strpos($post->category, 'Cho thuê màn hình LED') !== false || strpos($post->category, 'Dịch vụ thương mại') !== false || strpos($post->category, 'Thi công PhotoBooth và Backdrop') !== false || strpos($post->category, 'Tổ chức sự kiện') !== false || strpos($post->category, 'Quay phim - Chụp hình sự kiện') !== false ? 'selected' : '' }}>
                        Dịch vụ</option>
                </select>
            </div>
